...
void TrajectoryActionServer::actionCallback(GoalHandle goal)
{
  ...
  if (hasCurrentGoal())
  {
    ROS_ERROR("Currently executing a trajectory, can't accept another one");
    goal.setRejected();
    return;
  }
...
void TrajectoryActionServer::playTrajectorySimple(const trajectory_msgs::JointTrajectory traj,
    const std::vector<int>& joint_indices, const int group)
{
  ROS_INFO("Playing trajectory with simple implementation");
  ...
  for (int i = 0; i < traj.points.size(); i++)
  {
    ...
    // ROS_INFO_STREAM("Setting target values for point " << i);
    valueLock.lock();
